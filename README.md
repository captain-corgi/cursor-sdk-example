...
- **PRs from public forks and Dependabot PRs do not run this job by default.** GitHub does not expose `CURSOR_API_KEY` to those workflows, so the bundled job uses `if: ${{ secrets.CURSOR_API_KEY != '' }}` and shows **Skipped** instead of failing with `missing required env`. Human / same-repo PRs with the secret configured run normally. Fork coverage with reviews still needs `pull_request_target` or a `workflow_run` split — see the [fork FAQ in `USAGE.md`](USAGE.md#faq).
