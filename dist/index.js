...
import { autoApprove, commentOnPR, getPullRequestDiffTextForFormat, getPullRequestSnapshot, listOpenBotReviewThreadIds, listPriorSummaryCommentIds, makeOctokit, minimizeComments, openAutofixPr, requestCodeownersReview, resolveReviewThreads, SUMMARY_COMMENT_MARKER, updatePullRequestTitleAndBody, } from "./github.js";
...
async function main() {
    ...
    try {
        ...
        console.log(`[orchestrator] pr labels: ${env.ctx.labels.length ? env.ctx.labels.join(", ") : "(none)"}`);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(`[orchestrator] pr snapshot failed: ${msg}`);
    }
    const hasLabel = (l) => env.ctx.labels.includes(l);
    ...
    if (formatDisabled) {
        console.log(`[orchestrator] '${LABELS.DISABLE_FORMAT}' label present; skipping format step.`);
    }
    else if (isAutofixBranch) {
        console.log(`[orchestrator] head '${env.ctx.headRef}' is an autofix branch; skipping format step.`);
    }
    else {
        try {
            let diffSummary;
            try {
                ...
                console.log(`[orchestrator] format diff context: ${diffSummary.length} chars`);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
            ...
            if (fmt.error) {
                ...
                console.log(`[orchestrator] format applied: ${fmt.notes ?? "(no notes)"}`);
            }
            else {
                console.log(`[orchestrator] format unchanged: ${fmt.notes ?? "(no notes)"}`);
            }
        }
        catch (err) {
    ...
    if (!hasLabel(LABELS.REVIEW)) {
        console.log(`[orchestrator] '${LABELS.REVIEW}' label missing; skipping (no-op).`);
        return EX_OK;
    }
    let priorThreadIds = [];
    try {
        priorThreadIds = await listOpenBotReviewThreadIds(octokit, env.ctx);
        console.log(`[orchestrator] prior bot review threads: ${priorThreadIds.length}`);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(`[orchestrator] list prior review threads failed: ${msg}`);
    }
    let priorSummaryCommentIds = [];
    try {
        priorSummaryCommentIds = await listPriorSummaryCommentIds(octokit, env.ctx, SUMMARY_COMMENT_MARKER);
        console.log(`[orchestrator] prior bot summary comments: ${priorSummaryCommentIds.length}`);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
    ...
    if (priorThreadIds.length > 0) {
        try {
            const { resolved, failed } = await resolveReviewThreads(octokit, priorThreadIds);
            console.log(`[orchestrator] resolved prior threads: ${resolved} (failed: ${failed})`);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    console.log(`[orchestrator] complexity=${review.result.complexity} findings=${review.result.findings.length}`);
    ...
    if (autofixable.length > 0 && hasLabel(LABELS.AUTOFIX)) {
        ...
        if (autofix.attempted && autofix.branch && !autofix.error) {
            try {
                const prInfo = await openAutofixPr(octokit, env.ctx, autofix.branch, `autofix: review findings for #${env.ctx.prNumber}`, buildAutofixPrBody(env.ctx, autofixable));
                autofix.fixPrUrl = prInfo.url;
                console.log(`[orchestrator] autofix PR ${prInfo.reused ? "reused" : "opened"}: ${prInfo.url}`);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
        ...
        console.log(`[orchestrator] '${LABELS.AUTOFIX}' label missing; skipping autofix (${autofixable.length} autofixable findings).`);
    }
    let linearUrl;
    if (blocking.length > 0 &&
        ...
        console.log(`[orchestrator] '${LABELS.LINEAR}' label missing; skipping Linear issue creation.`);
    }
    ...
    if (priorSummaryCommentIds.length > 0) {
        try {
            const { minimized, failed } = await minimizeComments(octokit, priorSummaryCommentIds);
            console.log(`[orchestrator] minimized prior summary comments: ${minimized} (failed: ${failed})`);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    const safeToAutoApprove = review.result.complexity === "low" &&
        blocking.length === 0 &&
        (autofix.attempted ? Boolean(autofix.fixPrUrl) : true);
    if (safeToAutoApprove) {
        await autoApprove(octokit, env.ctx, buildApprovalBody(review.result.summary, autofix));
...
function buildApprovalBody(summary, autofix) {
    const lines = [
...
function buildAutofixPrBody(ctx, autofixable) {
    const lines = [
    ...
    for (const f of autofixable) {
        ...
        lines.push(`- **[${f.id}] ${f.title}** (severity: ${f.severity}) — \`${f.file}${lineSuffix}\``);
    }
    lines.push("", `Note: the autofix agent may have skipped findings it could not safely apply. Check the commit messages for any \`skipped\` notes.`);
    return lines.join("\n");
}
async function postSummaryComment(octokit, ctx, data) {
    ...
    const lines = [
        ...
        `- **Findings:** ${data.review.findings.length} (autofixable: ${data.review.findings.filter((f) => f.autofixable).length}, blocking: ${data.review.findings.filter((f) => !f.autofixable).length})`,
        `- **Labels:** ${labelsLine}`,
    ];
...
main()
    ...
    if (err instanceof CursorAgentError) {
        console.error(`[orchestrator] startup failed: ${err.message} (retryable=${err.isRetryable})`);
